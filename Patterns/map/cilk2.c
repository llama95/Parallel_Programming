...
int main(int argc, char** argv) {
  ...
  if(argc != 2) {
    ...
    int i; // declare for cilkfor loop
    int notfound=1;
    char passmatch[9]; // buffer for the matched password           
    cilk_for( i = 0; i < 1000000000; i++){
      char passmatchThreads[9]; // make pass
      genpass(i,passmatchThreads); // check match
      notfound=test(argv[1], passmatchThreads); // input file needed for arg
      if (notfound == 0){
      memcpy(passmatch,passmatchThreads,sizeof(passmatchThreads));
    }
