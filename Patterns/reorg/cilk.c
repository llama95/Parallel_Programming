...
place_uniformly(int sx, int ex, int sy, int ey, int sz, int ez, struct volume* v) {
  int counter = 0;
  ...
  for(int i=sx; i<=ex; i++) {
        for(int j=sy; j<=ey; j++) {
            for(int k=sz; k<=ez; k++) {
                ...
                v->x[i] = i; //array pts to each coord counter
                v->y[j] = j; // array will point to x,y,z malloc with sizeof float
                v->z[k] = k;
                v->mass[i] = 1; //
                v->mass[i] = fabs(v->x[i])+fabs(v->y[j])+fabs(v->z[k]);
...
void post_process(struct volume* v, float* cx, float* cy,int* length
    ...
    //double wx=0.0;
    //double wy=0.0;
    CILK_C_REDUCER_OPADD(mass_sum, float, 0.0);
    CILK_C_REDUCER_OPADD(wx, float, 0.0);
    CILK_C_REDUCER_OPADD(wy, float, 0.0);
